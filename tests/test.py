...
ax.plot(T_DB, label='direct beam')
ax.plot(T_DIF, label='diffuse transmissivity for diffuse incident')
ax.plot(T_DIR, label='diffuse transmissivity for direct incident')
ax.legend()
ax.set_title('transmissivities')
ax = fig.add_subplot(2,1,2)
ax.plot(R_DIF, label='diffuse reflectivity for diffuse incident')
ax.plot(R_DIR, label='diffuse reflectivity for direct incident')
ax.legend()
