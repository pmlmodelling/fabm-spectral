...
module fabm_spectral_atmosphere
   ...
   type,extends(type_particle_model), public :: type_spectral_atmosphere
      type (type_surface_diagnostic_variable_id) :: id_swr_sf, id_par_sf, id_uv_sf, id_par_E_sf, id_swr_dif_sf, id_mean_wind_out, id_wind_out, id_zen
      type (type_surface_diagnostic_variable_id) :: id_swr_sf_w, id_par_sf_w, id_uv_sf_w, id_par_E_sf_w
      type (type_surface_diagnostic_variable_id) :: id_alpha_a, id_beta_a, id_omega_a, id_F_a
      type (type_surface_diagnostic_variable_id) :: id_costheta_r
      type (type_surface_dependency_id) :: id_lon, id_lat, id_cloud, id_wind_speed, id_airpres, id_relhum, id_lwp, id_O3, id_WV, id_visibility, id_air_mass_type
      type (type_horizontal_dependency_id) :: id_mean_wind_speed
      type (type_global_dependency_id) :: id_yearday
      integer :: nlambda
      type (type_surface_diagnostic_variable_id), dimension(:), allocatable :: id_surface_band_dir, id_surface_band_dif
      type (type_surface_diagnostic_variable_id), dimension(:), allocatable :: id_direct, id_diffuse
...
contains
   ...
   subroutine initialize(self, configunit)
      class (type_spectral_atmosphere), intent(inout), target :: self
      ...
      real(rk), dimension(:), allocatable :: a_w_ip, b_w_ip
      ...
      if (compute_mean_wind) then
         ...
         call self%register_diagnostic_variable(self%id_mean_wind_out, 'mean_wind', 'm/s', 'daily mean wind speed')
      else
         call self%register_dependency(self%id_mean_wind_speed, 'mean_wind', 'm/s', 'daily mean wind speed')
      end if

      call self%register_diagnostic_variable(self%id_zen, 'zen', 'degrees', 'zenith angle')
      call self%register_diagnostic_variable(self%id_costheta_r, 'costheta_r', '-', 'cosine of zenith angle of underwater direct irradiance')

      ! Aerosol properties
      call self%register_diagnostic_variable(self%id_alpha_a,  'alpha_a', '-',   'aerosol Angstrom exponent', standard_variable=type_horizontal_standard_variable('angstrom_exponent_of_ambient_aerosol_in_air', '-'))
      call self%register_diagnostic_variable(self%id_beta_a,   'beta_a',  '-',   'aerosol scale factor for optical thickness')
      call self%register_diagnostic_variable(self%id_omega_a,  'omega_a', '-',   'aerosol single scattering albedo', standard_variable=type_horizontal_standard_variable('single_scattering_albedo_in_air_due_to_ambient_aerosol_particles', '-'))
      call self%register_diagnostic_variable(self%id_F_a,      'F_a',     '-',   'aerosol forward scattering probability')
      call self%register_diagnostic_variable(self%id_wind_out, 'wind',    'm/s', 'wind speed')
      ...
      ! (used to infer foam reflectance below)
      allocate(a_w_ip(self%nlambda), b_w_ip(self%nlambda))
      call interp(nlambda_w, lambda_w, a_w, self%nlambda, self%lambda, a_w_ip)
      call interp(nlambda_w, lambda_w, b_w, self%nlambda, self%lambda, b_w_ip)
      ...
      do l = 1, self%nlambda
         if (self%lambda(l) < 900) then
            ...
            log_T_w = -(a_w_ip(l) + 0.5_rk * b_w_ip(l))
      ...
      allocate(self%id_direct(size(self%lambda)))
      allocate(self%id_diffuse(size(self%lambda)))
      do l = 1, size(self%lambda)
         write(strindex, '(i0)') l
   ...
   subroutine do_surface(self, _ARGUMENTS_DO_SURFACE_)
      class (type_spectral_atmosphere), intent(in) :: self
      ...
      ! For debugging
      _SET_SURFACE_DIAGNOSTIC_(self%id_wind_out, wind_speed)
      if (_VARIABLE_REGISTERED_(self%id_mean_wind_out)) _SET_SURFACE_DIAGNOSTIC_(self%id_mean_wind_out, WM)
      ...
      _SET_SURFACE_DIAGNOSTIC_(self%id_zen, rad2deg * theta)
      costheta = cos(theta)
      ...
      _SET_SURFACE_DIAGNOSTIC_(self%id_alpha_a, alpha_a)
      _SET_SURFACE_DIAGNOSTIC_(self%id_beta_a, beta_a)
      _SET_SURFACE_DIAGNOSTIC_(self%id_omega_a, omega_a)
      _SET_SURFACE_DIAGNOSTIC_(self%id_F_a, F_a)
      ...
      _SET_SURFACE_DIAGNOSTIC_(self%id_par_sf, par_J)  ! Photosynthetically Active Radiation (W/m2)
      _SET_SURFACE_DIAGNOSTIC_(self%id_par_E_sf,par_E) ! Photosynthetically Active Radiation (umol/m2/s)
      _SET_SURFACE_DIAGNOSTIC_(self%id_swr_sf,  swr_J) ! Total shortwave radiation (W/m2) [up to 4000 nm]
      _SET_SURFACE_DIAGNOSTIC_(self%id_uv_sf, uv_J)    ! UV (W/m2)

      swr_J = sum(self%swr_weights * diffuse)
      _SET_SURFACE_DIAGNOSTIC_(self%id_swr_dif_sf, swr_J)

      select case (self%spectral_output)
      case (1)
         do l = 1, self%nlambda
            _SET_SURFACE_DIAGNOSTIC_(self%id_surface_band_dir(l), direct(l))
            _SET_SURFACE_DIAGNOSTIC_(self%id_surface_band_dif(l), diffuse(l))
         end do
      case (2)
         call interp(self%nlambda, self%lambda, direct, size(self%lambda_out), self%lambda_out, spectrum_out)
         do l = 1, size(self%lambda_out)
            _SET_SURFACE_DIAGNOSTIC_(self%id_surface_band_dir(l), spectrum_out(l))
         end do
         call interp(self%nlambda, self%lambda, diffuse, size(self%lambda_out), self%lambda_out, spectrum_out)
         do l = 1, size(self%lambda_out)
            _SET_SURFACE_DIAGNOSTIC_(self%id_surface_band_dif(l), spectrum_out(l))
         end do
      end select

      ! Sea surface reflectance
      call reflectance(self%nlambda, self%F, theta, wind_speed, rho_d, rho_s, costheta_r)
      _SET_SURFACE_DIAGNOSTIC_(self%id_costheta_r, costheta_r)
      ...
      do l = 1, self%nlambda
         _SET_SURFACE_DIAGNOSTIC_(self%id_direct(l), direct(l))
         _SET_SURFACE_DIAGNOSTIC_(self%id_diffuse(l), diffuse(l))
      end do
      ...
      par_E = sum(self%par_E_weights * spectrum)
      uv_J  = sum(self%uv_weights * spectrum)
      _SET_SURFACE_DIAGNOSTIC_(self%id_par_sf_w, par_J)  ! Photosynthetically Active Radiation (W/m2)
